...
void* performProducer(void* x) {
    ...
    while(args->numOps > 0) {
        ...
        sem_wait(&empty);
        sem_wait(&mutex);
        transactions[write_index] = transaction;
        write_index = (write_index + 1) % BUFFER_SIZE;
        sem_post(&mutex);
        sem_post(&full);
        args-> numOps--;
...
int main(int argc, char* argv[]) {
    ...
    for (i=0; i<NUM_PRODUCERS; ++i) {
        ...
        ret = pthread_create(&producer[i], NULL, performProducer, arg);
    ...
    for (j=0; j<NUM_CONSUMERS; ++j) {
        ...
        ret = pthread_create(&consumer[j], NULL, performConsumer, arg);
        if (ret != 0) { fprintf(stderr, "Error %d in pthread_create\n", ret); exit(EXIT_FAILURE); }
    }
