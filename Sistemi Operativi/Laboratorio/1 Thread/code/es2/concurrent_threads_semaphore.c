...
void* thread_work(void *arg) {
	int i;
	for (i = 0; i < m; i++){
		sem_wait(&sem); //acquisizione del lock
		shared_variable += v; //sezione critica
		sem_post(&sem); //rilascio del lock
	}
	return NULL;
}
...
int main(int argc, char **argv)
{
	...
	//Inizializzazione del semafoto con valore iniziale 1 (mutua esclusione)
	if(sem_init(&sem, 0, 1) != 0) {
		perror("Apertura sbagliata del semaforo");
		exit(EXIT_FAILURE);
	}
