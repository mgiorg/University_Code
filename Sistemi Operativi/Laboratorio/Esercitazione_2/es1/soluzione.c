...
void* performTransactions(void* x) {
    ...
    while (args->numOps > 0) {
        ...
        //! Importante questa sezione del codice
        sem_wait(&e);  // wait if buffer is full
        sem_wait(&s1); // enter in critical section
...
int main(int argc, char* argv[]) {
    ...
    //TODO: Prima di fare tutto dichiaro i semafori
    sem_init(&s1, 0, 1);
    sem_init(&s2, 0, 1);
    sem_init(&n, 0, 0);
    sem_init(&e, 0, BUFFER_SIZE);  // buffer is full at the beginning

    // TODO: Poi creo i thread e li assegno le funzioni e gli argomenti necessari
    ...
    // TODO: Poi faccio join su tutti i thread
    // join on threads
    for (i=0; i<NUM_PRODUCERS; ++i) {
