...
subject to tipi_minimi_materiale {j in MATERIALI}:
  sum{i in MODELLI, k in SPESSORI} y[i,j,k] >= NumMinTipiMateriale;

subject to tipi_minimi_spessore {k in SPESSORI}:
  sum{i in MODELLI, j in MATERIALI} y[i,j,k] >= NumMinTipiSpessore;

# Vincolo disgiuntivo 1: Almeno il 30% di "nature"
subject to disj1:
  sum{j in MATERIALI, k in SPESSORI} Quantita["nature",j,k] >= 0.3*NumMaterassi - M*z[1];

# Vincolo disgiuntivo 2: Nessun "comfort" in "memory" con spessore 40 cm
subject to disj2:
  Quantita["comfort","memory","40cm"] <= 0 + M*z[2];

# Vincolo disgiuntivo 3: Al massimo 5 combinazioni di "essential"
subject to disj3:
  sum{j in MATERIALI, k in SPESSORI} y["essential",j,k] <= 5 + M*z[3];

# Vincolo disgiuntivo complessivo
