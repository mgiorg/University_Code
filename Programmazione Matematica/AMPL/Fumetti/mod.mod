...
subject to maxTempo {r in REP}: #tutto in secondi
	sum{f in FORMATO, e in EROE} Consumo[f,r]*Quantita[f,e] <= MaxTempo[r]*3600;
subject to pagine {e in EROE}:
	MinPagine[e] <= sum{f in FORMATO} Quantita[f,e] <= MaxPagine[e];
subject to minPercEco:
	sum{i in ECO, e in EROE}Quantita[i,e] >= PercMinEco*sum{f in FORMATO, e in EROE} Quantita[f,e];
subject to minPercDel:
	sum{i in DEL, e in EROE}Quantita[i,e] >= PercMinDeluxe*sum{f in FORMATO, e in EROE} Quantita[f,e];