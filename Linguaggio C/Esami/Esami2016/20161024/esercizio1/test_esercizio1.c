...
int main(int argc, char** argv) {  
  ...
  for(i = 0; i < total_test_number; ++i) {
    ...
    Mat* src_mat = Mat_rand((rand() % 10) + 1, (rand() % 10) + 1, 10, 0);
    printf("src ");
    fflush(stdout);
    Mat_print(src_mat);
    fflush(stdout);

    Mat* gt_dest_mat = Mat_alloc(src_mat->cols, src_mat->rows);
    Mat* dest_mat = Mat_alloc(src_mat->cols, src_mat->rows);
    ...
    mat_rotate_90_clockwise(dest_mat, src_mat);
    printf("output ");
    fflush(stdout);
    Mat_print(dest_mat);
    fflush(stdout);
