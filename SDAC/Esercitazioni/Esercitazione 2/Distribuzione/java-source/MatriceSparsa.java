public class MatriceSparsa {
    ...
    public void set(int i, int j, int x) {
        if(i >= this.m || (i == this.m && j >= this.n)) {
            System.out.println("Gli estremi superano i limiti della matrice!");
            return;
        }
        if(x == 0) {
            mat_remove(i, j);
    ...
    public static MatriceSparsa tra(MatriceSparsa mat) {
		...
		MatriceSparsa ret = new MatriceSparsa(row, col);
		Elem it = mat.head;
		while(it != null) {
    ...
    public static MatriceSparsa mul(MatriceSparsa mat1, MatriceSparsa mat2) {
		...
		if(col1 != row2) {
			throw new RuntimeException("Impossibile moltiplicare le due matrici");
		}
		...
		System.out.println("Dimensioni matrice: " + ret.getNumRow() + " " + ret.getNumCol());
		return ret;
    }
}